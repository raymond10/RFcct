...
$wf2 = {			version : '0.5.4',
			...
			prefillSelectElements : function() {
				var select, selects = $wf2.getElementsByTagNames.apply(
						document.documentElement, [ 'select', 'datalist' ]);
			...
			addRepetitionBlock : function(refNode, index) {
				...
				if ((handlerAttr = this.getAttribute('onadded'))
						&& (!this.onadded || typeof this.onadded != 'function')) {
			...
			removeRepetitionBlock : function() {
				...
				if (this.repetitionTemplate != null) {
					...
					try {
						...
						removeEvt.initRepetitionEvent('removed', true, false,
								this);
			...
			updateMoveButtons : function(parent) {
				...
				for (i = 0; block = rbs[i]; i++) {
					...
					for (j = 0; btn = moveUpBtns[j]; j++) {
						btn.disabled = !(rb = $wf2.getRepetitionBlock(btn))
								|| (i == 0);
					}
			...
			controlCheckValidity : function() {
				...
				if (oninvalidAttr
						&& (!this.oninvalid || typeof this.oninvalid != 'function'))
			...
			updateValidityState : function(node) {
				...
				var doRequiredCheck = (doMaxLengthCheck || isFileInput
						|| type == 'email' || type == 'url' || type == 'text'
						|| type == 'password' || isRadioOrCheckbox);
				if (type == 'range') {
					...
					if ((!valueAttrNode || !valueAttrNode.specified)
							&& node.value === '' && !node.wf2ValueProvided) {
...
invalidMessages : {
			...
			getFormElements : function() {
				...
				var allElements = $wf2.getElementsByTagNames.apply(this, [
						'input', 'output', 'select', 'textarea', 'button' ]);
		...
		(function() {
			...
			if (/WebKit/i.test(navigator.userAgent)) {
				...
				document
						.write("<scr"
								+ "ipt id='__wf2_ie_onload' defer src='//:'><\/script>");
		...
		(function() {
			...
			function handleRepetitionEvent(evt) {
				...
				var handlerAttr = rt.getAttribute(attrName)
						|| rt.getAttribute(deprecatedAttrs[evt.type]);
				if (handlerAttr
						&& (!rt[attrName] || typeof rt[attrName] != 'function'))
