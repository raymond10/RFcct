...
if (!window.$wf2) {
	...
	if (document.implementation && document.implementation.hasFeature
		...
		$wf2 = {
			...
			_79 : function() {
				var select, selects = $wf2._50.apply(document.documentElement,
						[ 'select', 'datalist' ]);
				for ( var i = 0; select = selects[i]; i++) {
					...
					if (_23
							&& _23.documentElement
							&& /:?\bselect$/i
									.test(_23.documentElement.nodeName)
			...
			_68 : function() {
				...
				if (!this.addRepetitionBlock)
					this.addRepetitionBlock = function(_c, index) {
				...
				if (frm
						&& (_templateElements = $wf2._50.apply(this, [
				...
				while (_f && _f != this) {
					if (_f.nodeType == 1
							&& (_4 = _f.getAttributeNode('repeat'))
							&& /^-?\d+$/.test(_4.value)
							&& !_f.getAttribute('repeat-template')) {
			...
			initRepetitionBlocks : function(parentNode) {
				var repetitionBlocks = $wf2._51.apply(
						(parentNode || document.documentElement), [ [ '*' ],
				...
				if (!this.firstChild)
					this
							.appendChild(document
									.createTextNode($wf2.repetitionButtonDefaultLabels[_7]));
				if (_7 != 'add')
			...
			initRepetitionButtons : function(_7, _2) {
				...
				for (i = 0; i < inpts.length; i++) {
					...
					for ( var j = 0, _4; _4 = inpts[i].attributes[j]; j++)
						btn.setAttribute(_4.nodeName, inpts[i]
								.getAttribute(_4.nodeName));
			...
			_7f : function(_c, index) {
				...
				var _22 = this.getAttribute('id') ? 'id' : this
				...
				if (!_c) {
					_c = this;
					while (_c.previousSibling
							&& _c.previousSibling.repetitionType != 2)
			...
			_81 : function() {
				...
				if (this.repetitionTemplate != null) {
					...
					var _2d;
					if ((_2d = this.repetitionTemplate
						...
						this.repetitionTemplate.onremoved = new Function(
								'event', _2d)
					} else if ((_2d = this.repetitionTemplate
					...
					try {
						if (this.repetitionTemplate.onremoved) {
							...
							this.repetitionTemplate.onremove
									.apply(this, [ _d ])
						}
					} catch (err) {
						setTimeout(function() {
			...
			_82 : function(_5) {
				...
				if (this.repetitionTemplate) {
					...
					while (pos < rp.length && rp[pos] != this)
						pos++;
					rp.splice(pos, 1);
					rp.splice(_5 < 0 ? Math.max(pos + _5, 0) : Math.min(pos
							+ _5, rp.length), 0, this)
				}
				if (_5 < 0) {
					...
					while (_5 != 0 && target.nextSibling
							&& target.nextSibling.repetitionType != 1) {
			...
			_4e : function(button) {
				...
				var node;
				if (_4 && (node = document.getElementById(_4))
						&& node.getAttribute('repeat') == 'template')
					return node;
				return null
			},
			...
			_73 : function(_2) {
				...
				for (i = 0; block = rbs[i]; i++) {
					var moveUpBtns = $wf2._51.apply(block, [ [ 'button' ],
							'type', 'move-up' ]);
					for (j = 0; btn = moveUpBtns[j]; j++) {
						btn.disabled = !(rb = $wf2._4d(btn)) || (i == 0)
					}
					var moveDownBtns = $wf2._51.apply(block, [ [ 'button' ],
							'type', 'move-down' ]);
					for (j = 0; btn = moveDownBtns[j]; j++) {
						btn.disabled = !(rb = $wf2._4d(btn))
								|| (i == rbs.length - 1)
					}
				}
			},
			initNonRepetitionFunctionality : function(_2) {
				...
				var ctrl, ctrls = $wf2._50.apply(_2, [ 'input', 'select',
						'textarea' ]);
				...
				for (i = 0; i < els.length; i++)
					$wf2._5b(els[i]);
				var textareas = $wf2._51.apply(_2, [ [ 'textarea' ],
						'maxlength' ]);
			...
			_5b : function(el) {
				...
				while (node && node.nodeType == 1) {
					if ($wf2._53(node, 'visibility') == 'hidden'
							|| $wf2._53(node, 'display') == 'none')
						return;
			...
			_5e : function() {
				...
				try {
					if (this.oninvalid) {
						canceled = this.oninvalid.apply(this, [ _6 ]) === false
								|| canceled
					}
				} catch (err) {
					setTimeout(function() {
			...
			_6a : function(node) {
				...
				var _42 = (_2b || _36 || type == 'email' || type == 'url'
						|| type == 'text' || type == 'password' || _34);
				if (type == 'range') {
					node.min = (_10 && $wf2._63.test(_10.value)) ? Number(_10.value)
							: 0;
					if ((!_29 || !_29.specified) && node.value === ''
							&& !node.wf2ValueProvided) {
				...
				if (!node.validity.valueMissing && node.value) {
					...
					if (_77) {
						...
						_76.multiline = false;
						if (_76)
							node.validity.patternMismatch = !_76
									.test(node.value)
					}
					if (_2e || _15)
						node.validity.typeMismatch = ((node.wf2Value = $wf2
								._56(node.value, type)) == null);
					else {
						switch (type) {
						case 'number':
						case 'range':
							node.validity.typeMismatch = !$wf2._63
									.test(node.value);
							break;
						case 'email':
							node.validity.typeMismatch = !$wf2._6d
									.test(node.value);
							break;
						case 'url':
							node.validity.typeMismatch = !$wf2._6b
									.test(node.value);
							break
						}
					}
					if (!node.validity.patternMismatch
						...
						if (_41) {
							if (_31) {
								if (type == 'range') {
									node.max = (_11 && $wf2._63.test(_11.value)) ? Number(_11.value)
											: 100
								} else {
						...
						if (_20 && !node.validity.rangeUnderflow
							...
							if (!stepAttrNode) {
								...
								node.step = 'any';
							else if ($wf2._63.test(stepAttrNode.value)
							...
							if (node.step != 'any') {
								...
								if (_10)
									...
									node.wf2StepDatum = $wf2._43[type] ? $wf2._43[type]
											: 0;
								var _step = node.step;
								if (type == 'month') {
			...
			_47 : function(target) {
				...
				if (target.validity.typeMismatch)
					ol.appendChild($wf2._48($wf2.invalidMessages.typeMismatch
							.replace(/%s/, type)));
				if (target.validity.rangeUnderflow)
					ol.appendChild($wf2._48($wf2.invalidMessages.rangeUnderflow
							.replace(/%s/, $wf2._6c(target.min, type))));
				if (target.validity.rangeOverflow)
					ol.appendChild($wf2._48($wf2.invalidMessages.rangeOverflow
							.replace(/%s/, $wf2._6c(target.max, type))));
				if (target.validity.stepMismatch)
					ol
							.appendChild($wf2
									._48($wf2.invalidMessages.stepMismatch
											.replace(
													/%s/,
													target.step
															+ ($wf2.stepUnits[type] ? ' '
				...
				if (target.validity.tooLong)
					ol
							.appendChild($wf2
									._48($wf2.invalidMessages.tooLong
				...
				if (target.validity.patternMismatch)
					ol.appendChild($wf2
							._48($wf2.invalidMessages.patternMismatch.replace(
									/%s/, target.title ? target.title : ' "'
											+ target.getAttribute('pattern')
											+ '"')));
				...
				if (ol.childNodes.length == 1)
					ol.className = 'single';
				msg.appendChild(ol);
				var _2 = document.body ? document.body
						: document.documentElement;
				if ($wf2._44.length)
					_2
							.insertBefore(msg,
									$wf2._44[$wf2._44.length - 1].errorMsg);
				else
					...
					$wf2._46 = setInterval(
							function() {
								...
								for ( var i = 0; _1f = $wf2._44[i]; i++) {
									if (!_1f.target.className
											.match(/\bwf2_invalid\b/)) {
										_1f.target.className += " wf2_invalid"
									} else {
										_1f.target.className = _1f.target.className
				...
				case 1:
					...
					for (i = 0; _4 = node.attributes[i]; i++) {
						if ((_4.specified || $wf2._4a[_4.name])
							...
							if (_12 < 2 && (_4.name.indexOf('on') === 0)
									&& (typeof node[_4.name] == 'function')) {
					...
					if (node.className) {
						var _className = (_1 ? _1(node.className)
								: node.className);
					...
					if (node.style && node.style.cssText) {
						clone.style.cssText = (_1 ? _1(node.style.cssText)
								: node.style.cssText)
					}
					...
					for (i = 0; i < node.childNodes.length; i++) {
						clone
								.appendChild($wf2._7d(node.childNodes[i], _1,
										_12))
					}
					break;
				case 3:
			...
			_4f : function() {
				var elements = [];
				var _39 = $wf2._50.apply(this, [ 'input', 'output', 'select',
						'textarea', 'button' ]);
			...
			_62 : function(el) {
				...
				try {
					if (_2f = uri.match(/^data:[^,]*xml[^,]*,((?:.|\n)+)/)) {
						var xml = decodeURI(_2f[1].replace(/%3D/ig, '=')
			...
			_50 : function() {
				...
				if (document.evaluate) {
					...
					els = document.evaluate(_tagNames.join('|'), this, null, 7,
							null);
			...
			_51 : function(_38, _3, _8, _35) {
				...
				if (document.evaluate) {
					...
					els = document.evaluate(xPaths.join('|'), this, null, 7,
							null);
					for (i = 0; i < els.snapshotLength; i++)
						_1e.push(els.snapshotItem(i))
				} else {
					for (i = 0; i < _38.length; i++) {
						...
						for (j = 0; el = els[j]; j++) {
							...
							if (!_3
									|| (thisAttrNode && (_8 === undefined || (_35 ? _3e != _8
											: _3e == _8)))) {
								_1e.push(el)
							}
						}
			...
			_59 : function(date, type) {
				...
				if (date.getUTCMilliseconds())
					ms = '.'
							+ $wf2._58(date.getUTCMilliseconds(), 3).replace(
									/0+$/, '');
				switch (type) {
				case 'date':
					return date.getUTCFullYear() + '-'
							+ $wf2._58(date.getUTCMonth() + 1) + '-'
							+ $wf2._58(date.getUTCDate());
				case 'datetime-local':
					return date.getFullYear() + '-'
							+ $wf2._58(date.getMonth() + 1) + '-'
							+ $wf2._58(date.getDate()) + 'T'
							+ $wf2._58(date.getHours()) + ':'
							+ $wf2._58(date.getMinutes()) + ':'
							+ $wf2._58(date.getMinutes()) + ms + 'Z';
				case 'month':
					return date.getUTCFullYear() + '-'
							+ $wf2._58(date.getUTCMonth() + 1);
				case 'week':
					...
					return date.getUTCFullYear()
							+ '-W'
							+ $wf2
									._58(((date.valueOf() - week1.valueOf()) / (7 * 24 * 60 * 60 * 1000)) + 1);
				case 'time':
					return $wf2._58(date.getUTCHours()) + ':'
							+ $wf2._58(date.getUTCMinutes()) + ':'
							+ $wf2._58(date.getUTCMinutes()) + ms;
				case 'datetime':
				default:
					return date.getUTCFullYear() + '-'
							+ $wf2._58(date.getUTCMonth() + 1) + '-'
							+ $wf2._58(date.getUTCDate()) + 'T'
							+ $wf2._58(date.getUTCHours()) + ':'
							+ $wf2._58(date.getUTCMinutes()) + ':'
							+ $wf2._58(date.getUTCMinutes()) + ms + 'Z'
				}
			},
			_70 : function(_25) {
		...
		if (document.addEventListener) {
			document.addEventListener('DOMNodeInsertedIntoDocument', function(
					_6) {
				if (_6.target.nodeType == 1
						&& _6.target.hasAttribute('autofocus')) {
					$wf2._5b(_6.target)
				}
			}, false);
		...
		(function() {
			...
			if (/WebKit/i.test(navigator.userAgent)) {
				...
				document.write("<script defer src='" + $wf2._6f
